...
Collection.prototype.find = function find(criteria, cb) {
  ...
  if(query.aggregate) {
    ...
    return collection.aggregate(aggregate, function(err, cursor) {
      ...
      cursor.toArray(function(er, results) {
...
Collection.prototype.insert = function insert(values, cb) {
  ...
  this.connection.db.collection(this.identity).insertMany(docs, function(err, results) {
    if(err) return cb(err);
...
Collection.prototype.update = function update(criteria, values, cb) {
  ...
  collection.find(query.criteria.where, {projection: {_id: 1}}).toArray(function(err, records) {
    ...
    // Update the records
    collection.updateMany(query.criteria.where, { '$set': values }, { multi: true }, function(err, result) {
      if(err) return cb(err);
...
Collection.prototype.destroy = function destroy(criteria, cb) {
  ...
  var collection = this.connection.db.collection(self.identity);
  collection.deleteMany(query.criteria.where, function(err, results) {
    if(err) return cb(err);
